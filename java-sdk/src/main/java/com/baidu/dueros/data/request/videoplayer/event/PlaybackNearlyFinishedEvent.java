...
/**
 * 当当前video item播放快要结束，准备缓冲/下载播放队列中的下一个video item时会上报此事件，
 * 服务端可以返回Play指令，添加新的video item到队列末尾。
 * 
...
@JsonTypeName("VideoPlayer.PlaybackNearlyFinished")
public class PlaybackNearlyFinishedEvent extends VideoPlayerEvent {
    ...
    /**
     * 返回一个用来构造{@code PlaybackNearlyFinishedEvent}的{@code Builder}
     * 
     * @return Builder 用来构造{@code PlaybackNearlyFinishedEvent}
     */
     ...
     *            当前的播放进度
     */
    private PlaybackNearlyFinishedEvent(@JsonProperty("requestId") final String requestId,
    ...
    private PlaybackNearlyFinishedEvent(final Builder builder) {
        super(builder);
        token = builder.token;
    ...
    /**
     * 用来构造{@code PlaybackNearlyFinishedEvent}
     * 
     * @author hujie08(hujie08@baidu.com)
     * @version V1.0
     * @since 2018年5月7日
     */
    public static final class Builder extends RequestBodyBuilder<Builder, PlaybackNearlyFinishedEvent> {
        ...
        /**
         * 调用{@code PlaybackNearlyFinishedEvent}的私有构造方法构造{@code PlaybackNearlyFinishedEvent}
         * 
         * @see com.baidu.dueros.data.request.RequestBody.RequestBodyBuilder#build()
         */
        public PlaybackNearlyFinishedEvent build() {
            return new PlaybackNearlyFinishedEvent(this);
        }
    }
