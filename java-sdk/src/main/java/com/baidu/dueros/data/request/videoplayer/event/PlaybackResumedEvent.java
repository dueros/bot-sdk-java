...
/**
 * 在视频播放时，如果发生对话/闹钟等行为，则视频播放会暂停，此时，端上会的PlaybackResumed事件
 * 
...
@JsonTypeName("VideoPlayer.PlaybackResumed")
public class PlaybackResumedEvent extends VideoPlayerEvent {
    ...
    /**
     * 返回一个用来构造{@code PlaybackResumedEvent}的{@code Builder}
     * 
     * @return Builder 用来构造{@code PlaybackResumedEvent}
     */
     ...
     *            当前的播放进度
     */
    private PlaybackResumedEvent(@JsonProperty("requestId") final String requestId,
    ...
    private PlaybackResumedEvent(final Builder builder) {
        super(builder);
        token = builder.token;
    ...
    /**
     * 用来构造{@code PlaybackResumedEvent}
     * 
     * @author hujie08(hujie08@baidu.com)
     * @version V1.0
     * @since 2018年5月7日
     */
    public static final class Builder extends RequestBodyBuilder<Builder, PlaybackResumedEvent> {
        ...
        /**
         * 调用{@code PlaybackResumedEvent}的私有构造方法构造{@code PlaybackResumedEvent}
         * 
         * @see com.baidu.dueros.data.request.RequestBody.RequestBodyBuilder#build()
         */
        public PlaybackResumedEvent build() {
            return new PlaybackResumedEvent(this);
        }
    }
